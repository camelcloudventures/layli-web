    ...
    // If the action was dropped onto another action, we need to handle reordering
      ...
      if (activeAction && overAction) {
        ...
        if (activeAction.status !== overAction.status) {
          ...
          toast.success(`Action moved to ${overAction.status.replace('_', ' ')}`)
        }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage action items and follow-ups</p>
        </div>
        ...
        >
          ...
          <TabsContent value="board" className="mt-6">
            <DndContext collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
              <ActionBoard actions={actions} onEditAction={handleEditAction} />
              <DragOverlay>
                {activeAction ? (
                  <div className="w-[calc(100%-2rem)] max-w-sm">
                    <ActionCard action={activeAction} onClick={() => {}} isDraggable={false} />
                  </div>
                ) : null}
              </DragOverlay>
            </DndContext>
          ...
          <TabsContent value="list" className="mt-6">
                <ActionList actions={actions} onEditAction={handleEditAction} onDeleteAction={handleDeleteAction} />
          </TabsContent>
        </Tabs>
      </div>
