...
const serializeInspection = (inspection: Inspection): string => {
  return JSON.stringify(inspection, (key, value) => {
    if (key === "conducted_on" || key === "last_modified" || key === "completed_on" || key === "dueDate") {
...
const deserializeInspection = (data: string): Inspection => {
  ...
  try {
    if (typeof window !== "undefined") {
      ...
      if (!storedInspections) {
        localStorage.setItem("inspections", serializeInspection(mockInspections as any))
      }
    }
  } catch (error) {
...
export const getAllInspections = async (): Promise<Inspection[]> => {
  try {
    if (typeof window !== "undefined") {
      ...
      const storedInspections = localStorage.getItem("inspections")
      if (storedInspections) {
        const inspections: Inspection[] = JSON.parse(storedInspections, (key, value) => {
          if (key === "conducted_on" || key === "last_modified" || key === "completed_on" || key === "dueDate") {
            return value ? new Date(value) : value
          }
          return value
...
export const createInspection = async (inspection: Omit<Inspection, "id">): Promise<Inspection> => {
  try {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("inspections", serializeInspection(updatedInspections as any))
    }
...
export const updateInspection = async (inspection: Inspection): Promise<Inspection> => {
  try {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("inspections", serializeInspection(inspections as any))
    }
...
export const createDefaultInspection = (name: string, locationId: string): Inspection => {
  const location = mockLocations.find((loc) => loc.id === locationId) || mockLocations[0]
